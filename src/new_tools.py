...
def contribution_others(robots, robot1, leader_follower, arg_target_index, simulation_tick, followers_sp, params, OBSTACLES, target_obs):
    ...
    if leader_follower == 'for_leader':
        ...
        # for obs_idx in range(1, 4): #we will remove one by one: original
        for obs_idx in range(1, 2): #we will remove one by one: original # for rebuttal
        ...
        temp_max_obs_ltr = temp_max_obs_dia#np.linalg.norm(after_normal_cal_sp - temp_sp[6]) # for rebuttal
        # temp_sp[7] it's about wall
        temp_wall = temp_max_obs_ltr #np.linalg.norm(after_normal_cal_sp - temp_sp[7]) # for rebuttal
